...
class GCodeContext:
    def __init__(self, xy_feedrate, z_feedrate, start_delay, stop_delay, pen_up_angle, pen_down_angle, z_height, finished_height, x_home, y_home, register_pen, num_pages, continuous, file):
      ...
      self.postscript = [
        "",
				"M3S%0.2F (pen up)" % self.pen_up_angle,
				"G4 P%0.2F (wait %dms)" % (self.stop_delay, self.stop_delay),
				"M5 (turn off servo)",
      ...
      self.registration = [
        "M3S%d (pen down)" % (self.pen_down_angle),
        "G4 P%0.2F (wait %dms)" % (self.start_delay, self.start_delay),
        "M3S%d (pen up)" % (self.pen_up_angle),
        "G4 P%0.2F (wait %dms)" % (self.stop_delay, self.stop_delay),
    ...
    def start(self):
      self.codes.append("M3S%0.2F (pen down)" % self.pen_down_angle)
      self.codes.append("G4 P%0.2F (wait %dms)" % (self.start_delay, self.start_delay))
      self.drawing = True

    def stop(self):
      self.codes.append("M3S%0.2F (pen up)" % self.pen_up_angle)
      self.codes.append("G4 P%0.2F (wait %dms)" % (self.stop_delay, self.stop_delay))
      self.drawing = False
    ...
    def draw_to_point(self, x, y, stop=False):
      ...
      if stop:
        ...
        if self.drawing == False:
            self.codes.append("M3S%0.2F (pen down)" % self.pen_up_angle)
            self.codes.append("G4 P%0.2F (wait %dms)" % (self.start_delay, self.start_delay))
            self.drawing = True
